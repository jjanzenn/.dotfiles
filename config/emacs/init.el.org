...
Configure =mu4e= as an email client.
#+begin_src emacs-lisp
  (use-package mu4e :ensure ( :host github
    ...
    (setq mail-user-agent 'mu4e-user-agent
          mu4e-drafts-folder "/jjanzen/Drafts"
          mu4e-sent-folder "/jjanzen/Sent"
          mu4e-trash-folder "/jjanzen/Trash"
          mu4e-sent-messages-behavior 'delete
          mu4e-maildir-shortcuts '((:maildir "/jjanzen/INBOX" :key ?i)
                                   (:maildir "/jjanzen/Sent" :key ?s)
                                   (:maildir "/jjanzen/Trash" :key ?t)
                                   (:maildir "/jjanzen/All Mail" :key ?a))
          mu4e-get-mail-command "mbsync --all"
          user-mail-address "me@jjanzen.ca"
          user-full-name "j. janzen")

    :config
    (add-to-list 'mu4e-bookmarks '(:query "maildir:/jjanzen/INBOX" :name "Inbox" :key ?i :favorite t)))
#+end_src
...
Install =kdl-mode= for interacting with KDL config files.
#+begin_src emacs-lisp
