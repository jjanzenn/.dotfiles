...
#+begin_src sh :tangle ~/.dotfiles/install :mkdirp yes
  ...
  osascript -e "tell application \"System Events\" to tell every desktop to set picture to \"/Users/jjanzen/.wallpaper\" as POSIX file"
#+end_src
...
#+begin_src sh :tangle ~/.dotfiles/install :mkdirp yes
  if ! ps -e | sed '/grep/d' | grep 'emacs.*--daemon'; then
      launchctl load -w ~/Library/LaunchAgents/gnu.emacs.daemon.plist
  fi
#+end_src

Make Emacs.app executable (why does Mac OS make this so painful?).

#+begin_src sh :tangle ~/.dotfiles/install :mkdirp yes
  chmod +x ~/Applications/Emacs.app/Contents/MacOS/Emacs
#+end_src
